...
if lambda == 0  || isempty(penalty_set) == 1 || sum(weight_pen) == 0  % no penalty
    [hat_beta, converge] = robust_GLM_BD_parameter_estimate(...
        I_loss, family, link, ...
        X, y, beta_0, options, index_robust_c, c_tune_constant, w_robust);
    return;
end
...
if (index_robust_c == 0 || c_tune_constant == inf) && var(w_robust) == 0
    ...
    [hat_beta, converge] = GLM_BD_CD_penalized_parameter_estimate(...
        I_loss, family, link, ...
        weight_pen, penalty_set, lambda, X, y, beta_0, options);
    return
end
...
if any(isempty(hat_beta)) == 1
    ...
    disp(' !!!robust_GLM_BD_CD_penalized_parameter_estimate.m: some estimate of beta = [ ]!!!');
end

if any(isnan(hat_beta)) == 1
    disp(['I_loss = ', num2str(I_loss)]);
    disp(' !!!robust_GLM_BD_CD_penalized_parameter_estimate.m: some estimate of beta = NaN!!!');
end

if any(isinf(hat_beta)) == 1
    disp(['I_loss = ', num2str(I_loss)]);
    disp(' !!!robust_GLM_BD_CD_penalized_parameter_estimate.m: some estimate of beta = Inf!!!');
end
