...
function [hat_beta, converge] = GLM_BD_CD_penalized_parameter_estimate_SCAD_LLA(...
    I_loss, family, link, penalty_set, lambda, X, y, beta_0, options)
%--------------------------------------------------------------------------
% Name     : GLM_BD_CD_penalized_parameter_estimate_SCAD_LLA.m
...
%            GLM_BD_CD_penalized_parameter_estimate_SCAD_LLA_LM.m,
%            true_penalty.m,
...
if I_loss == 3 && strcmpi(link, 'iden')  % quadratic loss, identity link
    [hat_beta, converge] = GLM_BD_CD_penalized_parameter_estimate_SCAD_LLA_LM(...
...
while converge == false && iter <= options.maxit && ...
    ...
    for j = penalty_set
        if     hat_beta(j) ~= 0
            w(j) = true_penalty(hat_beta(j), I_penalty, lambda, 1, ...
                options)*sign(hat_beta(j))/lambda;
        elseif hat_beta(j) == 0
            w(j) = 1;
        end
    end
...
if any(isempty(hat_beta)) == 1
    ...
    disp(' !!!GLM_BD_CD_penalized_parameter_estimate_SCAD_LLA.m: some estimate of beta = [ ]!!!');
end

if any(isnan(hat_beta)) == 1
    disp(['I_loss = ', num2str(I_loss)]);
    disp(' !!!GLM_BD_CD_penalized_parameter_estimate_SCAD_LLA.m: some estimate of beta = NaN!!!');
end

if any(isinf(hat_beta)) == 1
    disp(['I_loss = ', num2str(I_loss)]);
    disp(' !!!GLM_BD_CD_penalized_parameter_estimate_SCAD_LLA.m: some estimate of beta = Inf!!!');
end
