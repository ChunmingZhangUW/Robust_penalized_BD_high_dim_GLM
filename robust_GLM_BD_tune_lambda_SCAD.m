...
function [lambda_opt, min_TE, Hat_beta, I] = robust_GLM_BD_tune_lambda_SCAD(...
    I_loss, family, link, X, y, beta_0, X_test, y_test, penalty_set, ...
    n_grid_lambda, grid_A, grid_B, misclass, options, ...
    index_robust_c, choice_rho_function, c_tune_constant, w_robust)

%--------------------------------------------------------------------------
% Name     : robust_GLM_BD_tune_lambda_SCAD.m
...
% Called   : robust_GLM_BD_CD_penalized_parameter_estimate_SCAD.m,
%            robust_GLM_BD_LARS_penalized_parameter_estimate_SCAD.m,
%            robust_true_pp.m
%--------------------------------------------------------------------------
% <Input>
...
%  beta_0    : K\times 1, initial estimate of beta
...
for k = 1:n_grid_lambda
    ...
    if     options.choice_initial_value == 0
        if     options.CD_LARS == 1
            beta_1 = robust_GLM_BD_CD_penalized_parameter_estimate_SCAD(...
                I_loss, family, link, penalty_set, lambda, X, y, ...
                beta_0, options, index_robust_c, ...
                choice_rho_function, c_tune_constant, w_robust);

        elseif options.CD_LARS == 2
            beta_1 = robust_GLM_BD_LARS_penalized_parameter_estimate_SCAD(...
                I_loss, family, link, penalty_set, lambda, X, y, ...
                beta_0, options, index_robust_c, ...
                choice_rho_function, c_tune_constant, w_robust);
        end
    ...
    if misclass == 1 % for Bernoulli responses
        ...
        predict_BD = robust_true_pp(I_loss, family, link, hat_theta_test, ...
            y_test, 0, options, index_robust_c, ...
