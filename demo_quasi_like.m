...
%---------------------------------------------------------------------
% file_name = ['demo_quasi_like', ...
%     '_', num2str(phi), ...
%     '_', num2str(study), ...
%     '_', num2str(index_robust(1)), '_', num2str(index_robust(2)), ...
%     '_', num2str(options.choice_of_Poisson_phi_in_robust_p_1_p_2_BD_for_hat_V_n), ...
%     '_', num2str(options.specified_Poisson_phi_in_robust_p_1_p_2_BD_for_hat_V_n), ...
%     '_', options.choice_rho_function, ...
%     '_', c_tune_constant_character, ...
%     '_', num2str(options.choice_of_weight_function_in_robust_BD_estimation), ...
%     '_', num2str(options.GLM_BD_parameter_estimate), ...
%     '_', num2str(options.robust_GLM_BD_parameter_estimate), ...
%     '_', num2str(options.GLM_BD_CD_penalized_parameter_estimate), ...
%     '_', num2str(options.robust_GLM_BD_CD_penalized_parameter_estimate), ...
%     '_A', num2str(A), ...
%     '_', num2str(n_common), ...
%     '_', num2str(num_sim)];
% %'_', num2str(options.c_tune_constant), ...

% output_file_name = [file_name, '_output.txt'];
% fN = exist(output_file_name, 'file');
% if fN == 2
%     delete(output_file_name)
% end
% eval(['diary ', output_file_name])
...
for cases = 1:length(num_obs_set)
    ...
    %######################## Part 1 ############################
    %rand('twister', seeds_set(cases));
    %randn('state',  seeds_set(cases));
    ...
    if lambda_method == 1 % CV method
        SEED_1 = unidrnd(10^4,num_sim, 1);  SEED_2 = unidrnd(10^4,num_sim, 1);
    end
    ...
    for sim = 1:num_sim
        ...
        if     lambda_method == 0 % tuning set
            [lambda, Test_Error_L1, Hat_beta, I] = robust_GLM_BD_tune_lambda(...
                ...
                n_grid_lambda, grid_A, grid_B, misclass, options, ...
                index_robust(1), choice_rho_function, c_tune_constant, w_robust);
            hat_beta_L1 = Hat_beta(:, I);

        elseif lambda_method == 1 % CV method
            [lambda, CVE_L1] = robust_GLM_BD_CV_lambda(seed_1, seed_2, ...
                I_loss, family, link, ...
                design_matrix, y_vector, beta_0, weight_pen, penalty_set, ...
                n_grid_lambda, grid_A, grid_B, n_subset, misclass, options, ...
                index_robust, choice_rho_function, c_tune_constant);

            if     options.CD_LARS == 1
                hat_beta_L1 = robust_GLM_BD_CD_penalized_parameter_estimate(...
                    I_loss, family, link, weight_pen, penalty_set, lambda, ...
                    design_matrix, y_vector, ...
                    beta_0, options, index_robust(1), choice_rho_function, ...
        ...
        elseif lambda_method == 1 % CV method
            [lambda, CVE_SCAD] = robust_GLM_BD_CV_lambda_SCAD(seed_1, seed_2, ...
                I_loss, family, link, ...
                design_matrix, y_vector, beta_0, penalty_set, ...
                n_grid_lambda, grid_A, grid_B, n_subset, misclass, options, ...
                index_robust, choice_rho_function, c_tune_constant);
            ...
            if     options.CD_LARS == 1
                hat_beta_SCAD = ...
                    ...
                    I_loss, family, link, penalty_set, lambda, ...
                    design_matrix, y_vector, beta_0, ...
                    options, index_robust(1), choice_rho_function, ...
                    c_tune_constant, w_robust);
            end
        end % if lambda_method == 0 or 1
        ...
        elseif lambda_method == 1 % CV method
            [lambda, CVE_CR] = robust_GLM_BD_CV_lambda(seed_1, seed_2, ...
                I_loss, family, link, design_matrix, y_vector, ...
                beta_0, weight_pen, ...
                penalty_set, ...
                n_grid_lambda, grid_A, grid_B, n_subset, misclass, options, ...
                index_robust, choice_rho_function, c_tune_constant);
            ...
            if     options.CD_LARS == 1
                hat_beta_CR = robust_GLM_BD_CD_penalized_parameter_estimate(...
                    I_loss, family, link, weight_pen, penalty_set, lambda, ...
                    design_matrix, y_vector, ...
                    beta_0, options, index_robust(1), choice_rho_function, ...
                    c_tune_constant, w_robust);
        ...
        for k = 1:n_grid_kappa
            ...
            if     lambda_method == 0 % tuning set
                [lambda, min_TE, Hat_beta, I] = robust_GLM_BD_tune_lambda(...
                    ...
                    grid_B, misclass, options, index_robust(1), ...
            ...
            elseif lambda_method == 1 % CV method
                [lambda, min_TE] = robust_GLM_BD_CV_lambda(seed_1, seed_2, ...
                    I_loss, family, link, design_matrix, y_vector, ...
                    beta_0, weight_pen, ...
                    penalty_set, ...
                    n_grid_lambda, grid_A, grid_B, n_subset, misclass, ...
                    options, index_robust, choice_rho_function, c_tune_constant);

                Weight_Pen(:, k) = weight_pen;
                Lambda(k) = lambda;
            end % if lambda_method == 0 or 1
        ...
        elseif lambda_method == 1 % CV method
            weight_pen = Weight_Pen(:, kappa_opt);        % optimal weights
            lambda = Lambda(kappa_opt); % optimal lambda

            if     options.CD_LARS == 1
                hat_beta_PCR = robust_GLM_BD_CD_penalized_parameter_estimate(...
                    I_loss, family, link, weight_pen, penalty_set, lambda, ...
                    design_matrix, y_vector, beta_0, options, index_robust(1), ...
                    choice_rho_function, c_tune_constant, w_robust);
            end
        end % if lambda_method == 0 or 1
...
% print(handle_figure, '-depsc', [file_name, '.eps']);
